...
export const sortByCountryFromLowestActions =
  (searchResults, countries) => async (dispatch) => {
    dispatch({
      type: SORT_FROM_LOWEST,
    });
    try {
      const data = await sortByCountryFromLowestService(
        searchResults,
        countries
      );
      dispatch({
...
export const sortByCountryFromHighestActions =
  (searchResults, countries) => async (dispatch) => {
    dispatch({
      type: SORT_FROM_HIGHEST,
    });
    try {
      const data = await sortByCountryFromHighestService(
        searchResults,
        countries
      );
      dispatch({
